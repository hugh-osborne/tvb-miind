...
"""
MIIND implementation of Wilson-Cowan
...
class Miind_WilsonCowan(ModelNumbaDfun):
    ...
    The Miind_WilsonCowan class implements the well known Wilson-Cowan model [WC_1972]

    .. math::
    \dot{E}_k &= \dfrac{1}{\tau_e} (-E_k  + (k_e - r_e E_k) \mathcal{S}_e (\left( c_{ee} E_k - c_{ei} I_k  + P_k \right) ))\\
    \dot{I}_k &= \dfrac{1}{\tau_i} (-I_k  + (k_i - r_i I_k) \mathcal{S}_i (\left( c_{ie} E_k - c_{ee} I_k  + Q_k \right) ))\\

    \mathcal{S}_e(x) &= \dfrac{1}{1-exp(-a_e (x - b_e))}\\
    \mathcal{S}_i(x) &= \dfrac{1}{1-exp(-a_i (x - b_i))},

    **Technical Notes** :

    miind_wilson_cowan.py depends on module libmiindwc.so and for MIIND to have
    been installed on the system. Installation instructions are available at
    ...
    **References**:
    ...
    def configure(self):
    	...
    	self.miindmodel.init([self.c_ee[0], self.c_ei[0],
